...
def create_functions(tokenizer, model, device='cpu'):
    ...
    def encode(
    text_sentence: str, add_special_tokens=True
    ...
    def create_node_class(text_sentence: str):
        ...
        class Node:
            ...
            def __find_next_token_list__(self):
                ...
                input_ids, mask_idx = encode(
                    f"{text_sentence}{self.curr_word}<mask>"
                )
    ...
    text_sentence, top_k=5, verbose=False
    ) -> list[str]:
        """
        Args:
        ...
        Node, Token = create_node_class(text_sentence)
        ...
        probs.sort()
        return [string for prob, string in probs] + chamadas
